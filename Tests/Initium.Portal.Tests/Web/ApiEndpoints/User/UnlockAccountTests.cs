...
{
    public class UnlockAccountTests
    {
        [Fact]
        public async Task HandleAsync_GivenRequestIsNotValid_ExpectResponseWithFailedStatus()
        {
            ...
            var endpoint = new UnlockAccount(mediator.Object, currentAuthenticatedUserProvider.Object);
            endpoint.ModelState.AddModelError("key", "error-message");

            var response = await endpoint.HandleAsync(new UnlockAccount.EndpointRequest());
        ...
        [Fact]
        public async Task HandleAsync_GivenAttemptToUnlockSelf_ExpectFailedResponse()
        {
            var mediator = new Mock<IMediator>();
            mediator.Setup(x => x.Send(It.IsAny<UnlockAccountCommand>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(ResultWithError.Ok<ErrorData>());
        ...
        [Fact]
        public async Task HandleAsync_GivenNoAuthenticatedUser_ExpectFailedResponse()
        {
            var mediator = new Mock<IMediator>();
            mediator.Setup(x => x.Send(It.IsAny<UnlockAccountCommand>(), It.IsAny<CancellationToken>()))
                .ReturnsAsync(ResultWithError.Ok<ErrorData>());
        ...
        public class EndpointRequestValidatorTests
        {
            ...
            {
                var request = new UnlockAccount.EndpointRequest()
                {
                    UserId = TestVariables.UserId,
                };
                var validator = new UnlockAccount.EndpointRequestValidator();
